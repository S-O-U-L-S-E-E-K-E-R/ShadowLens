...
const WorldviewRightPanel = React.memo(function WorldviewRightPanel({ effects, setEffects, setUiVisible }: { effects: any; setEffects: any; setUiVisible: any }) {
    ...
    const saveRecording = (chunks: Blob[]) => {
        ...
        a.href = url;
        a.download = `ShadowLens_${ts}.webm`;
    ...
    const startRecording = async () => {
        try {
            ...
            mr.onstop = () => {
                saveRecording(chunksRef.current);
                setRecording(false);
            };
            stream.getVideoTracks()[0].onended = () => {
                // User clicked browser's "Stop sharing" — stop recorder so onstop fires
    ...
    const stopRecording = () => {
        // Stop the recorder first (triggers onstop which saves)
        if (recorderRef.current && recorderRef.current.state !== 'inactive') {
            recorderRef.current.stop();
        }
    ...
    const elStr = `${pad2(Math.floor(elapsed / 3600))}:${pad2(Math.floor((elapsed % 3600) / 60))}:${pad2(elapsed % 60)}`;
    const now = new Date();
    const doy = Math.floor((now.getTime() - new Date(now.getFullYear(), 0, 0).getTime()) / 86400000);
    const orbVal = (47000 + doy).toString().padStart(5, '0');
    const passVal = (280 + (now.getHours() * 4 + Math.floor(now.getMinutes() / 15))).toString().padStart(3, '0');
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-3 mb-6 border border-gray-800 bg-black/40 backdrop-blur-md px-4 py-2 rounded-sm relative shadow-[0_4px_30px_rgba(0,0,0,0.5)] pointer-events-auto">
                ...
                {!recording ? (
                    <div className="w-3 h-3 bg-red-500 rounded-full cursor-pointer hover:bg-red-400 hover:shadow-[0_0_10px_rgba(239,68,68,0.8)] transition-all" title="Start recording" onClick={startRecording}></div>
                ) : (
                    <div className="w-3 h-3 bg-red-500 rounded-sm cursor-pointer animate-pulse hover:bg-red-400 transition-all" title="Stop recording" onClick={stopRecording}></div>
                )}
                <div className="text-[10px] font-mono text-gray-400 tracking-wider">
                    REC {currentTime.date} {currentTime.time}
                    {recording && <span className="text-red-400 ml-2">[{elStr}]</span>}
                    <br />
                    ORB: {orbVal} PASS: DESC-{passVal}
                </div>
            </div>
